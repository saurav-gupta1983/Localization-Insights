...
namespace Adobe.Localization.Insights.Web.UserControls
{
    ...
    public partial class MaintainLocalesVsPlatformsMatrixUserControl : System.Web.UI.UserControl
    {
            ...
            foreach (DataRow drMatrixData in dtMatrixData.Rows)
            {
                ...
                if (suiteID != "" && suiteID.Contains("TS_"))
                {
                    if (drMatrixData[WebConstants.COL_TEST_SUITE_RUN_IDS].ToString() == "")
                    {
                        ...
                        //ts.LocaleTSKeyID = dtProjectPhaseLocales.Select(WebConstants.COL_PROJECT_LOCALE_ID + "=" + drMatrixData[WebConstants.COL_PROJECT_LOCALE_ID].ToString())[0][WebConstants.COL_TEST_STUDIO_KEY_ID].ToString();
                        //ts.PlatformTSKeyID = dtProjectPhasePlatforms.Select(WebConstants.COL_PROJECT_PLATFORM_ID + "=" + drMatrixData[WebConstants.COL_PROJECT_PLATFORM_ID].ToString())[0][WebConstants.COL_TEST_STUDIO_KEY_ID].ToString();
                        //drMatrixData[WebConstants.COL_TEST_SUITE_RUN_IDS] = BO.BusinessObjects.CreateTestSuiteRuns(ts).TestSuiteRunID;
                    }
                    else if (drMatrixData[WebConstants.COL_TEST_SUITE_RUN_IDS].ToString().Length == COL_SUITE_ID_LENGTH)
                    {
        ...
        private void EnableDisableButtonDisplay()
        {
            ...
            ButtonGenerateSID.Enabled = isMatrixCreated;
            ...
            if (RadioButtonDocument.Checked)
            {
                ...
                ButtonGenerateSID.Enabled = false;
            }
            else
            {
                ButtonCreateTestSuite.Enabled = isMatrixCreated;
                ButtonGenerateSID.Enabled = true;
            }
        }
